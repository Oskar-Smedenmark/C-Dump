...
void insertFirst(Dlist	**first, Dlist *el)
{
    if(first == NULL)
    {
        printf("Error: first is NULL\n");
        return -1;
    }
    if(el == NULL)
    {
        printf("Error: el is NULL\n");
        return -1;
    }
    el->next = *first;
    if(*first != NULL)
...
void insertSorted(Dlist	**first, Dlist *el)
{
    if(first == NULL)
    {
        printf("Error: first is NULL\n");
    ...
    if((*first)->sensorData > el->sensorData) //Element should be first
    {
        el->next = *first;
        el->prev = NULL;
        (*first)->prev = el;
    ...
    while(temp != NULL)
    {
        if(temp->sensorData > el->sensorData) //Element should be in the middle of the list
        {
            el->prev = temp->prev;
            el->next = temp;
            temp->prev->next = el;
    ...
    if(last->sensorData < el->sensorData) //Element should be last in list
    {
        el->prev = last;
        last->next = el;
        el->next = NULL;
